...
Stats = namedtuple('Stats', [
    'tid',
    'true_adult_males', 'true_subadult_males', 'true_adult_females', 'true_juveniles', 'true_pups', 'true_total',
    'count_adult_males', 'count_subadult_males', 'count_adult_females', 'count_juveniles', 'count_pups', 'count_total',
    'diff_adult_males', 'diff_subadult_males', 'diff_adult_females', 'diff_juveniles', 'diff_pups', 'diff_total',
])
...
class SeaLionData(object):
    ...
    def __init__(self, sourcedir=SOURCEDIR, datadir=DATADIR, verbosity=VERBOSITY.NORMAL):
        ...
        self.paths = {
            ...
            'stats': os.path.join(datadir, 'stats.csv'),
        }
    ...
    def coords(self, train_id):
        ...
        if avg_diff > MAX_AVG_DIFF:
            print('Warning: Bad data for %d' % train_id)
        ...
        for cls, color in enumerate(self.cls_colors):
            ...
            has_color = np.sqrt(np.sum(np.square(
                dot_img * (img_diff > 0)[:, :, None] - color_array), axis=-1)) < MAX_COLOR_DIFF
        ...
        diff_counts = np.array(true_counts) - np.array(counts)
        stats = Stats(
            train_id,
            true_counts[0], true_counts[1], true_counts[2], true_counts[3], true_counts[4], np.sum(true_counts),
            counts[0], counts[1], counts[2], counts[3], counts[4], np.sum(counts),
            diff_counts[0], diff_counts[1], diff_counts[2], diff_counts[3], diff_counts[4], np.sum(diff_counts)
        )
        if self.verbosity >= VERBOSITY.VERBOSE:
            print()
            print('train_id', 'true_counts', 'counts', 'difference', sep='\t')
            print(train_id, true_counts, counts, diff_counts, sep='\t' )
        ...
        return sealions, stats

    def save_coords(self, train_ids=None): 
        if train_ids is None: train_ids = self.train_ids
        coord_fn = self.path('coords')
        stats_fn = self.path('stats')
        self._progress('Saving sealion coordinates to {}'.format(coord_fn))
        with open(coord_fn, 'w') as coord_csv, open(stats_fn, 'w') as stat_csv:
            writer_coord = csv.writer(coord_csv)
            writer_stat = csv.writer(stat_csv)
            writer_coord.writerow(SeaLionCoord._fields)
            writer_stat.writerow(Stats._fields)
            for tid in train_ids:
                self._progress()
                coords, stats = self.coords(tid)
                for coord in coords:
                    writer_coord.writerow(coord)
